...
#define CallSub(SubTaskName, ...) do {uint8_t currdt; _lc=(uint8_t)((__LINE__%255)+1); return 0; case (uint8_t)((__LINE__%255)+1):  currdt=SubTaskName(__VA_ARGS__); if(currdt!=0xFF) return currdt;} while(0);
